...
#ifdef DEBUG
                     if ( MyRank == 0 ) then
                        write(*,'(a,i6,a,i4,a,i6,a,i4,a,i6,a,i4,a,3i6)') &
...
!!
                     ...
                     WTimeBgn = MPI_WTIME()
                     CALL CPU_TIME(TimeBgn)
!coarray                     CALL MPI_Allreduce(RWork2, MOIntSome, ldc*n, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_MAT, IErr)
                     MOIntSome(1:ldc,1:n) = RWork2(1:ldc,1:n)
!!
                     CALL CPU_TIME(TimeEnd)
                     WTimeEnd = MPI_WTIME()

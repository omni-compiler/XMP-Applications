...
!$xmp align (i,j) with t(i,j) :: volume, density0, density1, energy0, energy1, pressure, viscosity, xarea, yarea, &
!$xmp                            xvel0, xvel1, yvel0, yvel1, volume_change
!$xmp shadow (2:2, 2:2) :: volume, density0, density1, energy0, energy1, pressure, viscosity
!$xmp shadow (2:3, 2:2) :: xarea
!$xmp shadow (2:2, 2:3) :: yarea
!$xmp shadow (2:3, 2:3) :: xvel0, xvel1, yvel0, yvel1, volume_change
!$xmp save_desc :: p, t, xarea, yarea, volume, density0, density1, energy0, energy1, pressure, viscosity, xvel0, xvel1, yvel0, yvel1, volume_change
!!$OMP PARALLEL

    IF(predict)THEN
!$xmp loop (j,k) on t(j,k)
!$OMP PARALLEL DO PRIVATE(right_flux,left_flux,top_flux,bottom_flux,total_flux,min_cell_volume,energy_change,recip_volume,volume_change_s)
      DO k=y_min,y_max
        DO j=x_min,x_max

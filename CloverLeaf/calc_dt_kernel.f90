...
!$xmp align (i,j) with t(i,j) :: volume, density0, energy0, pressure, viscosity_a, soundspeed, xarea, yarea, xvel0, yvel0, dt_min
!$xmp align (i) with t(i,*) :: cellx, celldx
!$xmp align (j) with t(*,j) :: celly, celldy
!$xmp shadow (2:2, 2:2) :: volume, density0, energy0, pressure, viscosity_a, soundspeed
!$xmp shadow (2:3, 2:2) :: xarea
!$xmp shadow (2:2, 2:3) :: yarea
!$xmp shadow (2:3, 2:3) :: xvel0, yvel0, dt_min
!$xmp shadow (2:2) :: cellx, celldx, celly, celldy
!$xmp save_desc :: p, t, xarea,yarea,celly,celldy,volume,density0,energy0,pressure,viscosity_a,soundspeed,xvel0,yvel0,dt_min
...
!$OMP PARALLEL DO PRIVATE(dsx,dsy,cc,dv1,dv2,div,dtct,dtut,dtvt,dtdivt) REDUCTION(MIN : dt_min_val)
    DO k=y_min,y_max
      DO j=x_min,x_max
...
!!$OMP END PARALLEL
    ...
    jldt = g_l_min_x - 1 + jldt
    kldt = g_l_min_y - 1 + kldt
    xl_pos=cellx(jldt)
    yl_pos=celly(kldt)

...
!$xmp nodes p(*,*)
!$xmp template tmplt(x_min-2:x_max+3,y_min-2:y_max+3)
!$xmp distribute tmplt(gblock(blocksize_x), gblock(blocksize_y)) onto p
!$xmp align (i) with tmplt(i,*) :: celldx
!$xmp align (j) with tmplt(*,j) :: celldy
!$xmp align (i,j) with tmplt(i,j) :: density0, pressure, viscosity, xvel0, yvel0
!$xmp shadow (2:2) :: celldx, celldy
!$xmp shadow (2:2, 2:2) :: density0, pressure, viscosity
!$xmp shadow (2:3, 2:3) :: xvel0, yvel0
!$xmp save_desc :: p, tmplt, celldx, celldy, density0, pressure, viscosity, xvel0, yvel0

!!$OMP PARALLEL

!$xmp loop (j,k) on tmplt(j,k)
!$OMP PARALLEL DO PRIVATE(ugrad,vgrad,div,strain2,pgradx,pgrady,pgradx2,pgrady2,limiter,pgrad,xgrad,ygrad,grad,grad2,dirx,diry)
    DO k=y_min,y_max
      DO j=x_min,x_max

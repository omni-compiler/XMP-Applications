...
!    real(8) :: v_pl(GRD_XDIR:GRD_ZDIR,ADM_VLINK_NMAX)
    ...
    !--- 2020 Fujitsu
    integer(8) :: vsend_pl_l_desc
    integer(8) :: vsend_pl_l_lb(2), vsend_pl_l_ub(2)
    integer(8) :: vsend_pl_l_sec
    !--- 2020 Fujitsu end
...
!coarray    integer :: istat(MPI_STATUS_SIZE)
    integer :: n, l, ierr
...
!coarray
    !--- 2020 Fujitsu
    !allocate(vrecv_plA(GRD_XDIR:GRD_ZDIR,5)[*])
    vrecv_plA_lb(1) = GRD_XDIR; vrecv_plA_ub(1) = GRD_ZDIR
    vrecv_plA_lb(2) = 1; vrecv_plA_ub(2) = 5
    call xmp_new_coarray(vrecv_plA_desc, 8, 2, vrecv_plA_lb, vrecv_plA_ub, 1, img_dims)
    call xmp_coarray_bind(vrecv_plA_desc, vrecv_plA)

    vsend_pl_l_lb(1) = GRD_XDIR; vsend_pl_l_ub(1) = GRD_ZDIR
    vsend_pl_l_lb(2) = 1; vsend_pl_l_ub(2) = ADM_VLINK_NMAX
    call xmp_new_local_array(vsend_pl_l_desc, 8, 2, vsend_pl_l_lb, vsend_pl_l_ub, loc(vsend_pl))

    call xmp_new_array_section(vsend_pl_l_sec, 2)
    call xmp_new_array_section(vrecv_plA_sec, 2)
    !--- 2020 Fujitsu end
    vrecv_pl (:,:) = 0.d0
    !vrecv_plc(:,:) = 0.d0 !--- 2020 Fujitsu ---
    vrecv_plA(:,:) = 0.d0
...
!coarray
          GRD_xt_pl(n+1,ADM_KNONE,ADM_NPL,:) = vrecv_plA(:,n)
       enddo
    endif
...
!          GRD_xt_pl(n+1,ADM_KNONE,ADM_SPL,:) = vrecv_pl(:,n) ! [mod] H.Yashiro 20120525
          GRD_xt_pl(n+1,ADM_KNONE,ADM_SPL,:) = vrecv_plA(:,n)
       enddo
    endif
    ...
    !--- 2020 Fujitsu
    call xmp_free_array_section(vsend_pl_l_sec)
    call xmp_free_array_section(vrecv_plA_sec)

    call xmp_coarray_deallocate(vrecv_plA_desc, ierr)
    call xmp_free_local_array(vsend_pl_l_desc)
    !--- 2020 Fujitsu end
    return

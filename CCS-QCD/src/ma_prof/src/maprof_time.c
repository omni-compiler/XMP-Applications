...
*/
//* 20180206: Calling C from Fortran
void maprof_time_start_(int *id)
{
  sections[*id].started = 1;
  sections[*id].start = get_current_time();
}
...
void maprof_add_fp_ops_(int *id, double *ops)
{
  maprof_add_fp_ops(*id,*ops);
}
...
void maprof_add_ld_ops_(int *id, double *ops)
{
  maprof_add_ld_ops(*id,*ops);
}
...
void maprof_add_st_ops_(int *id, double *ops)
{
  maprof_add_st_ops(*id,*ops);
}
...
void maprof_add_ld_min_ops_(int *id, double *ops)
{
  maprof_add_ld_min_ops(*id,*ops);
}
...
void maprof_add_st_min_ops_(int *id, double *ops)
{
  maprof_add_st_min_ops(*id,*ops);
}
...
void c_print_(int *id, const char *name)
{
  maprof_print(*id,name);
}
...
void c_print_time_(int *id, const char *name)
{
  maprof_print_time(*id,name);
}
...
void c_print_time_mpi_(int *id, const char *name)
{
  maprof_print_time_mpi(*id,name);
}
...
void c_print_time_mpi_full_(int *id, const char *name)
{
  maprof_print_time_mpi_full(*id,name);
}
#endif
